...
export default function Login({ onLoginSuccess, onSwitchToRegister, onSwitchToForgot }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Real API Call to Express/Postgres Backend
      ...
      // Save JWT token so Settings.jsx can retrieve it
      if (token) {
      ...
      // Handles incorrect username/password or missing user errors
      setError(
        err.response?.data?.error || 'Invalid credentials. Please check your username/email/phone and password.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md p-6 sm:p-8 bg-white rounded-2xl border border-slate-200 shadow-xl">
